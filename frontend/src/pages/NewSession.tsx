...
const DEFAULT_FORM: PersonaFormValues = {
...
const FINANCIAL_LABELS: Record<string, { label: string; color: string; bg: string }> = {
...
function getPreviewName(form: PersonaFormValues): string {
  const maleNames = [
    'James', 'Carlos', 'Marcus', 'Raj', 'Ahmed', 'Wei', 'Andre',
    'Diego', 'Ethan', 'Omar', 'Kenji', 'Isaiah', 'Vikram', 'Javier', 'Tae',
  ];
  const femaleNames = [
    'Jennifer', 'Maria', 'Priya', 'Aisha', 'Mei', 'Sofia', 'Keisha',
    'Elena', 'Nadia', 'Yuki', 'Fatima', 'Angela', 'Divya', 'Lucia', 'Imani',
  ];
  const lastNames = [
    'Thompson', 'Garcia', 'Patel', 'Nguyen', 'Kim', 'Robinson', 'Martinez',
...
function getSpousePreviewName(form: PersonaFormValues): string {
  const maleNames = [
    'William', 'Robert', 'Alejandro', 'Vikram', 'Hassan', 'Kenji', 'Andre',
    'Javier', 'Daniel', 'Tariq', 'Rahul', 'Ethan', 'Miguel', 'Isaiah', 'Wei',
  ];
  const femaleNames = [
    'Patricia', 'Elena', 'Priya', 'Fatima', 'Yuki', 'Sofia', 'Monique',
    'Deepa', 'Angela', 'Yasmin', 'Gabriela', 'Samantha', 'Nadia', 'Aisha', 'Lin',
  ];
  const lastNames = [
    'Williams', 'Martinez', 'Singh', 'Kim', 'Hassan', 'Chen', 'Robinson',
...
function getAgeRange(ag: string): string {
...
export default function NewSession() {
  const [form, setForm] = useState<PersonaFormValues>(DEFAULT_FORM);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const persona = {
        ...
        backstory,
        // Send both singular (old backend compat) and plural (new backend) forms
        employment_type: primaryEmployment,
        primary_concern: form.primary_concerns[0] ?? '',
        gender: form.gender as 'male' | 'female',
        age_group: form.age_group as 'young_adult' | 'middle_aged' | 'senior' | 'elderly',
        marital_status: form.marital_status as 'single' | 'married' | 'divorced' | 'widowed' | 'domestic_partner',
        financial_situation: form.financial_situation as 'struggling' | 'stable' | 'comfortable' | 'affluent' | 'wealthy' | 'ultra_wealthy',
        has_debt: form.has_debt as 'none' | 'manageable' | 'significant' | 'overwhelming',
        risk_tolerance: form.risk_tolerance as 'very_conservative' | 'conservative' | 'moderate' | 'aggressive' | 'very_aggressive',
        investment_experience: form.investment_experience as 'none' | 'beginner' | 'intermediate' | 'experienced' | 'expert',
        personality_type: form.personality_type as 'anxious' | 'confident' | 'skeptical' | 'analytical' | 'emotional' | 'impulsive' | 'detail_oriented' | 'trusting',
        communication_style: form.communication_style as 'direct' | 'reserved' | 'chatty' | 'formal',
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">Build your simulated client before starting the training session</p>
      </div>
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="bg-navy-800 border border-navy-700 rounded-xl p-6">
              ...
              <div className="space-y-5">
                {/* Gender — hidden for couple (each person's gender set in Spouse section) */}
                {!isCouple && (
                  <div>
